...
## Scope
Everything here is desk-based: the modeled Gilbert–Elliott channel, the gem5
memory path, and a labeled (non-adaptive) attacker. AEAD, metadata, and
write-persistence latencies are swept calibration inputs, not hardware
measurements, and the slow serialized-persistence runs are finite-run overload
observations rather than steady-state bounds. The gem5 controller leaves
attestation, trusted-verifier provisioning, and distributed zeroization out of
scope. The crash model checks abstract ordering, not a physical journal, so its
line-local invariants say nothing about refcount repair or erasure correctness.
None of this is a worst-case cryptographic proof; see the paper's Evaluation and
Discussion sections for the full scoping.
